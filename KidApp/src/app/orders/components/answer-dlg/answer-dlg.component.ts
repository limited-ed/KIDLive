...
export class AnswerDlgComponent implements OnInit {
  ...
  set order( order: Order ) {
    this._order = order;
    if ( order ) { this.form.setValue({answer: this.order.answer}); }
  }
  ...
  constructor(private fb: FormBuilder, private http: HttpClient, private orderSrv: OrderService) {
  ...
  onCancel(){
    ...
    this.http.patch(this.url + '/' + this._order.id , {},
                     { headers: {Authorization: 'Bearer ' + localStorage.getItem('id_token')}}).subscribe({
      complete: (() => { this.onFinish(); }).bind(this)
    });
  }
